...
function ezPrvtTableDrawLines($pos,$gap,$x0,$x1,$y0,$y1,$y2,$col,$inner,$outer,$opt=1,$borders=array()){
  $bordersDefault = array('top'=>1, 'right'=>1, 'botton'=>1, 'left'=>1);
  $borders = am($bordersDefault, $borders);
  ...
  foreach($pos as $x){
    ...
    $this->line($x-$gap/2,$y0-$gapY,$x-$gap/2,$y2);
    if ($x>$x1){ $x1=$x; };
    if ($x<$x0){ $x0=$x; };
  }
  $this->setLineStyle($outer);
...
//  $y-=$gap-$decender;
  ...
  while ($ok==0){
    ...
    if($optionsAll['separadorHeader']) {
	    $this->line(20,$y,578,$y);//linea arriba
	}
...
function ezTable(&$data,$cols='',$title='',$options=''){
  ...
  $defaults = array(
    'shaded'=>1,'showLines'=>1,'showLinesExtended'=>array(),'separadorHeader'=>false,'shadeCol'=>array(0.8,0.8,0.8),'shadeCol2'=>array(0.7,0.7,0.7),'fontSize'=>10,'titleFontSize'=>12, 'headerFontSize'=>10
  ...
  foreach($maxWidth as $colName => $w){
    ...
    if (!empty($options['cols'][$colName]['width'])) {
    	if(ereg("([0-9\.].\%$)", $options['cols'][$colName]['width'])) {
    		$options['cols'][$colName]['width'] = $options['width'] * str_replace("%", "", $options['cols'][$colName]['width']) / 100;
    	}
    }
  ...
  if ($options['showHeadings']){
    ...
    $headingHeight = $this->ezPrvtTableColumnHeadings($cols,$pos,$maxWidth,$height,$decender,$options['rowGap'],$options['headerFontSize'],$y,$options);
    $y0 = $y+$headingHeight;
    $y1 = $y;
    ...
    while(!$abortTable && ($newPage || $newRow)){
      ...
      if ($newPage || $y<$this->ez['bottomMargin'] || (isset($options['minRowSpace']) && $y<($this->ez['bottomMargin']+$options['minRowSpace'])) ){
        ...
        if ($options['showLines']){
          ...
          $this->ezPrvtTableDrawLines($pos,$options['gap'],$x0,$x1,$y0,$y1,$y2,$options['lineCol'],$options['innerLineThickness'],$options['outerLineThickness'],$options['showLines'], $options['showLinesExtended']);
        }
        if ($options['shaded']){
          $this->closeObject();
          $this->restoreState();
...
//        $x1=$x1+$dm;
        ...
        $mx=0;
        if ($options['showHeadings']){
          $this->ezPrvtTableColumnHeadings($cols,$pos,$maxWidth,$height,$decender,$options['rowGap'],$options['headerFontSize'],$y,$options);
          $y1=$y;
        } else {
          $y1=$y0;
        }
